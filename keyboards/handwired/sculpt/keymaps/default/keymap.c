...
*/
  ...
  [_MEDIA_KEYS] = LAYOUT( \
    _______,  KC_MY_COMPUTER,   KC_WWW_STOP,   KC_WWW_BACK,   KC_WWW_FORWARD,   KC_WWW_SEARCH,   KC_WWW_REFRESH,   KC_MEDIA_PREV_TRACK,   KC_MEDIA_NEXT_TRACK,   KC_MPLY,   KC_MUTE,  KC_VOLD,     KC_VOLU,  _______, _______, _______, _______, \
...
{
  setPinOutput(B5); //DEBUG LED
  setPinOutput(B7); //GREEN LED
  setPinInput(F4);  //Fn switch
}
...
void matrix_scan_user(void) {
  ...
  //toggling this pin provides inverted voltage to the green led
  led_state.scroll_lock ? togglePin(B7) : writePinLow(B7);
}

bool process_record_user(uint16_t keycode, keyrecord_t *record) {

  if(readPin(F4))
    layer_on(_MEDIA_KEYS);
  else
    layer_off(_MEDIA_KEYS);
